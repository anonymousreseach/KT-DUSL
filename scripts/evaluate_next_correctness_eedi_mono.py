...
"""
evaluate_next_correctness_eedi.py

EEDI online next-question correctness prediction evaluation (mono-tag).
...
Data:
- Interactions: train_task_1_2.csv with columns [UserId, QuestionId, IsCorrect]
- Hierarchy: subject_metadata.csv with columns [SubjectId, ParentId]
- Question metadata: question_metadata_task_1_2_with_single_leaf.csv mapping QuestionId -> SubjectId_single
...
# -------------------------
# EEDI: build hierarchy + q2s (mono-tag)
# -------------------------
...
def build_q2s_single_leaf(question_metadata_single_csv: str) -> Dict[int, List[int]]:
    df = pd.read_csv(question_metadata_single_csv)
    if "QuestionId" not in df.columns:
        raise KeyError(f"Missing QuestionId in {question_metadata_single_csv}. Columns={list(df.columns)}")

    sid_col = None
    for c in ["SubjectId_single", "SubjectIdSingle", "subject_id_single"]:
        if c in df.columns:
            sid_col = c
            break
    if sid_col is None:
        raise KeyError(f"Missing SubjectId_single in {question_metadata_single_csv}. Columns={list(df.columns)}")
...
def main() -> None:
    ...
    ap.add_argument("--question_metadata_single_csv", required=True)

    ap.add_argument("--max_users", type=int, default=300)

    # Sharding (post-df build, on UserId)
    ...
    q2s = build_q2s_single_leaf(args.question_metadata_single_csv)
    ...
    for uid in user_ids:
        ...
        for step, row in enumerate(udf.itertuples(index=False)):
            ...
            if args.out_predictions_csv is not None:
                # mono-tag: export concept ids + u/evidence consistently
                if sids:
                    sid = int(sids[0])
                    u_leaf = float(leaf_sl.opinion(sid)[2])
                    u_prop = float(state_sl[sid][2])
                    ...
                    concept_ids = [sid]
                    u_leaf_by_concept = {sid: u_leaf}
                    u_prop_by_concept = {sid: u_prop}
                    evidence_by_concept = {sid: ev}
                else:
                    concept_ids = []
                    u_leaf_by_concept = {}
                    u_prop_by_concept = {}
                ...
                pred_rows.append({
                    ...
                    "evidence_by_concept": json.dumps(evidence_by_concept, separators=(",", ":")),
    ...
    run_meta = {
        ...
        "question_metadata_single_csv": str(args.question_metadata_single_csv),
